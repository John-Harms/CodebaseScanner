...
class CodeScannerApp:
    def __init__(self, root):
        ...
        self.generate_directory_tree_var = tk.BooleanVar(value=True) # For Feature 4 GUI toggle

        self.rules_files = []
        self.rules_folders = []
        self.rules_dirty = False

        self.directory_tree_blacklist = [] # For Feature 2
        self.directory_tree_blacklist_dirty = False # Tracks changes to the tree blacklist
        ...
        root.rowconfigure(0, weight=1) # Allow main_frame to expand

        # Adapt main_frame grid to allow for more rows for GUI elements
        # We'll use current_row to keep track of where to place new elements.
        # main_frame.rowconfigure(X, weight=Y) will be used to manage expansion of rule lists
        ...
        self.filter_mode_check.pack(side=tk.LEFT, padx=(0, 20)) # Add some padding to separate
        ...
        self.generate_tree_check = ttk.Checkbutton(
            ...
            command=self._on_generate_tree_toggle # Optional: if any immediate action needed
        )
        ...
        # This allows the user to resize the rules list and tree blacklist sections
        ...
        self.rules_outer_frame = ttk.Frame(self.paned_window) # Frame to add to PanedWindow
        self.rules_frame = ttk.LabelFrame(self.rules_outer_frame, text="Scan Rules List", padding="5")
        self.rules_frame.pack(fill=tk.BOTH, expand=True) # Pack inside its container for PanedWindow
        ...
        self.rules_list_frame_inner = ttk.Frame(self.rules_canvas) # This frame holds the actual rule widgets
        ...
        self.paned_window.add(self.rules_outer_frame, weight=1) # Add rules section to paned window


        # --- Directory Tree Blacklist Frame (Scrollable) ---
        self.tree_blacklist_outer_frame = ttk.Frame(self.paned_window) # Frame to add to PanedWindow
        self.tree_blacklist_frame = ttk.LabelFrame(self.tree_blacklist_outer_frame, text="Directory Tree Blacklist", padding="5")
        self.tree_blacklist_frame.pack(fill=tk.BOTH, expand=True) # Pack inside its container
        ...
        self.tree_blacklist_frame.rowconfigure(0, weight=1) # Make the canvas area expandable
        ...
        # Note: No "Save" button here, tree blacklist is saved with the profile.

        self.paned_window.add(self.tree_blacklist_outer_frame, weight=1) # Add tree blacklist section
        ...
        run_button.grid(row=current_row, column=0, columnspan=3, pady=10) # This row is after the PanedWindow
        # current_row has already been incremented after PanedWindow grid
        style = ttk.Style()
        try:
    ...
    def _on_generate_tree_toggle(self):
        # This method is called when the "Generate Directory Tree" checkbox state changes.
        # We need to mark the profile as dirty if a profile is active.
        if self.active_profile_name:
            # A simple way to indicate profile data changed without a dedicated "profile_dirty" flag
            # is to ensure save_profile handles this new field.
            # For now, just logging or updating status if needed.
    ...
    def _configure_fields_from_initial_profile(self):
        ...
        if profile_to_load_name and profile_to_load_name in self.profiles:
            ...
            if profile_data:
                ...
                self.directory_tree_blacklist = list(profile_data.get("directory_tree_blacklist", [])) # Ensure it's a list copy
        ...
        self._rebuild_tree_blacklist_gui() # Ensure tree blacklist GUI is updated
    ...
    def _on_mousewheel(self, event):
        ...
        while current_widget is not None:
            if current_widget == self.rules_canvas:
                ...
                break
            if current_widget == self.tree_blacklist_canvas: # Check for tree blacklist canvas
    ...
    def _update_window_title(self):
        ...
        if self.active_profile_name:
            # Check if tree blacklist has unsaved changes when a profile is active
            # A more robust way would be a general "profile_dirty" flag.
            # For now, saving a profile always saves current tree blacklist state.
            profile_dirty_indicator = ""
            if self.directory_tree_blacklist_dirty: # Check only tree blacklist for now
                profile_dirty_indicator = "*"
    ...
    def _save_profile_dialog(self):
        ...
        if profile_name:
            ...
            # Rules file existence check is fine.

            self.profiles[profile_name] = {
                "scan_directory": os.path.normpath(scan_dir),
                "save_filepath": os.path.normpath(save_fp),
                "rules_filepath": os.path.normpath(rules_fp) if rules_fp else "", # Store empty string if no rules file
                "filter_mode": filter_mode,
                "directory_tree_blacklist": current_tree_blacklist, # Feature 3
                "generate_directory_tree": current_generate_tree_setting # Feature 3
            }
            self.last_active_profile_name = profile_name
            self.active_profile_name = profile_name
            self.directory_tree_blacklist_dirty = False # Reset dirty flag for tree blacklist after saving
    ...
    def _apply_profile_settings(self, profile_name, persist_last_active=False):
        ...
        if profile_data:
            ...
            self.directory_tree_blacklist_dirty = False # Reset dirty flag
            ...
            if normalized_new_rules_path != normalized_current_rules_path and self.rules_dirty:
                if messagebox.askyesno("Unsaved Rules", f"You have unsaved changes in the current scan rules list for\n'{os.path.basename(normalized_current_rules_path if normalized_current_rules_path else 'Untitled Rules')}'.\nDiscard changes to load rules from the new profile?", default=messagebox.NO, parent=self.root):
                    self.rules_dirty = False
                else:
                    self._update_status(f"Profile '{profile_name}' load cancelled to keep unsaved scan rule changes.")
                    return False
            ...
            if persist_last_active:
                 self.last_active_profile_name = profile_name
                 save_profiles(self.profiles, self.last_active_profile_name) # Save here to persist last_active_profile
    ...
    def _execute_delete_profile(self, profile_name_to_delete):
        if profile_name_to_delete in self.profiles:
            ...
            if self.active_profile_name == profile_name_to_delete:
                ...
                # Clear tree blacklist fields as well
                self.directory_tree_blacklist = []
                self.generate_directory_tree_var.set(True) # Reset to default
    ...
    def _initialize_rules_file_and_display(self):
        ...
        if filepath:
            ...
            if os.path.exists(normalized_filepath) and os.path.isfile(normalized_filepath):
                ...
                if prompt_create:
                    if create_empty_file(normalized_filepath, is_rules_file=True):
    ...
    def _browse_rules_file(self):
        ...
        if filepath_selected:
            ...
            if os.path.exists(normalized_new_filepath) and os.path.isfile(normalized_new_filepath):
                ...
                if create_empty_file(normalized_new_filepath, is_rules_file=True):
    ...
    def _load_and_display_rules(self, filepath):
        ...
        try:
            ...
            self.rules_dirty = False
            self._rebuild_rules_list_gui()
            self._update_status(f"Loaded {len(self.rules_files) + len(self.rules_folders)} scan rules from {os.path.basename(normalized_filepath)}")
        except Exception as e:
            messagebox.showerror("Load Error", f"Could not load or parse scan rules from {os.path.basename(filepath)}:\n{e}", parent=self.root)
            self._update_status(f"Error loading scan rules from {os.path.basename(filepath)}")
    ...
    def _rebuild_rules_list_gui(self):
        if not hasattr(self, 'rules_list_frame_inner'):
            print("Warning: _rebuild_rules_list_gui called before rules_list_frame_inner was initialized.")
            return
        ...
        if not self.rules_files and not self.rules_folders:
            placeholder_text = "No scan rules defined." if current_file_path else "No scan rules file selected or rules defined."
            ttk.Label(self.rules_list_frame_inner, text=placeholder_text, foreground="grey").grid(row=0, column=0, columnspan=2, padx=5, pady=5)

        self.rules_list_frame_inner.update_idletasks()
        self._on_frame_configure(None, self.rules_canvas)
        self._update_rules_frame_title()
    ...
    def _rebuild_tree_blacklist_gui(self):
        if not hasattr(self, 'tree_blacklist_list_frame_inner'):
            print("Warning: _rebuild_tree_blacklist_gui called before tree_blacklist_list_frame_inner was initialized.")
            return
        ...
        row_num = 0
        # Display tree blacklist items (always folders)
        if self.directory_tree_blacklist:
            ...
            row_num += 1
            for path_rule in sorted(self.directory_tree_blacklist): # Ensure sorted display
                item_frame = ttk.Frame(self.tree_blacklist_list_frame_inner)
                item_frame.grid(row=row_num, column=0, columnspan=2, sticky=tk.EW, padx=5, pady=1)
                item_frame.columnconfigure(0, weight=1)

                label_text = f"{path_rule}" # Just the path, it's always a folder
                max_len = 80 # Max length for display
                display_text = label_text if len(label_text) <= max_len else "..." + label_text[-(max_len-3):]
                tooltip_text = label_text if len(label_text) > max_len else None
            ...
            ttk.Label(self.tree_blacklist_list_frame_inner, text="No directories blacklisted from tree.", foreground="grey").grid(row=0, column=0, columnspan=2, padx=5, pady=5)

        self.tree_blacklist_list_frame_inner.update_idletasks()
        self._on_frame_configure(None, self.tree_blacklist_canvas)
        self._update_tree_blacklist_frame_title()


    def _add_tree_blacklist_folder_rule(self):
        """Adds a selected folder (full path) to the directory tree blacklist."""
        initial_dir_add = self.scan_directory.get() if self.scan_directory.get() and os.path.isdir(self.scan_directory.get()) else os.path.expanduser("~")
        folderpath_to_add = filedialog.askdirectory(
        ...
        if normalized_fp_add and normalized_fp_add not in self.directory_tree_blacklist:
            ...
            self.directory_tree_blacklist_dirty = True # Mark as dirty
            self._rebuild_tree_blacklist_gui()
            ...
            self._update_window_title() # To show dirty star if profile active
    ...
    def _remove_tree_blacklist_item(self, path_to_remove):
        """Removes a folder path from the directory tree blacklist."""
    ...
    def _update_tree_blacklist_frame_title(self):
        # The title for the tree blacklist frame itself is static: "Directory Tree Blacklist"
        # The "dirty" status for tree blacklist is reflected in the main window title if a profile is active.
        # No separate dirty star on the frame's title for simplicity.
        pass # Title is static, dirty status shown in main window title
    ...
    def _run_scan_thread(self, scan_dir_norm, save_path_norm, rules_files_for_scan, rules_folders_for_scan, filter_mode_val, used_rules_file_display_path, generate_tree_flag, tree_blacklist_for_scan):
        ...
        `generate_tree_flag` and `tree_blacklist_for_scan` are for directory tree generation.
        """
        try:
            with open(save_path_norm, "w", encoding="utf-8") as output_file:
                # --- Feature 1: Directory Tree Generation ---
                if generate_tree_flag:
                    self._update_status("Generating directory tree...") # Update status
                    tree_header = f"# Directory Tree for: {os.path.basename(scan_dir_norm)}\n\n"
                    # Ensure tree_blacklist_for_scan contains normalized paths
                    normalized_tree_blacklist = [os.path.normpath(p) for p in tree_blacklist_for_scan]
                    tree_structure = generate_directory_tree_text(scan_dir_norm, normalized_tree_blacklist)
                    ...
                    output_file.write("\n\n---\n\n") # Separator before main scan content
                ...
                if self.rules_dirty and used_rules_file_display_path:
    ...
    def _run_scan(self):
        ...
        # Get directory tree settings for the scan
        generate_tree_selected = self.generate_directory_tree_var.get()
        tree_blacklist_current = list(self.directory_tree_blacklist) # Use a copy for the scan thread
        ...
        scan_thread = threading.Thread(
            ...
            args=(norm_scan_dir_for_thread, norm_save_path_for_thread,
                  ...
                  generate_tree_selected, tree_blacklist_current), # Pass tree generation parameters
            daemon=True
        )
        scan_thread.start()
