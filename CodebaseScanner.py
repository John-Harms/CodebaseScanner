...
def should_process_item(item_path, is_file, rules_files, rules_folders, filter_mode, whitelisted_parent_folders):
    ...
    if filter_mode == FILTER_BLACKLIST:
        if is_file:
            ...
            if normalized_item_path in rules_folders:
                return False
            # Also, don't process if it's a subfolder of an excluded folder (relevant for recursion decision)
            # This part is more for deciding whether to *enter* a folder during traversal.
            # If an explicit folder rule matches, it's out.
            # If we are deciding for a folder item, and it's inside another excluded folder,
            # it means the parent was already excluded. This check might be redundant
            # if the traversal logic correctly stops at excluded folders.
            # However, keeping it for direct calls to should_process_item for a folder if that occurs.
            for folder_rule in rules_folders:
                 if normalized_item_path.startswith(folder_rule + os.sep): # if it's a subfolder of an excluded one
                     return False # This line might be less critical if traversal stops early
        return True # Not in any blacklist rule

    elif filter_mode == FILTER_WHITELIST:
        # If the item itself is within any of the whitelisted_parent_folders, it's allowed through.
        # This ensures that contents of explicitly whitelisted folders are processed.
        ...
        # If not within an already whitelisted parent, check explicit rules
        if is_file:
    ...
    # Default fallback (should ideally not be reached if filter_mode is always one of the two)
...
def process_directory(directory, output_file, rules_files, rules_folders, filter_mode, level=0, status_callback=None, whitelisted_ancestor_folders=None):
    ...
    # Update current_whitelisted_ancestors for this level
    ...
    if filter_mode == FILTER_WHITELIST and normalized_directory in rules_folders:
        if normalized_directory not in current_whitelisted_ancestors: # Add if this dir itself is a whitelisted folder
    ...
    try:
        ...
        # Determine if we should report this error based on filter mode
        ...
        if filter_mode == FILTER_WHITELIST:
            ...
            else: # Check if it's a descendant of a whitelisted folder
        ...
        # Report error if in blacklist mode (implies we wanted to scan it unless excluded)
        # OR if in whitelist mode AND this dir was supposed to be scanned
        if filter_mode == FILTER_BLACKLIST or is_dir_in_whitelisted_scope:
            ...
            content_written_for_this_branch = True # Content (error message) was written
        if status_callback:
            status_callback(f"Error reading: {normalized_directory} - {e}")
        return content_written_for_this_branch

    files_to_output = []
    dirs_to_recurse_info = [] # Store as dicts {'name': item_name, 'path': normalized_item_path, 'ancestors': list_for_next_level}
    ...
    for item_name in items:
        ...
        if should_process_item(normalized_item_path, is_file, rules_files, rules_folders, filter_mode, current_whitelisted_ancestors):
            if is_file:
                ...
                # For recursion, pass the *current* accumulated whitelisted ancestors
                dirs_to_recurse_info.append({'name': item_name, 'path': normalized_item_path, 'ancestors': list(current_whitelisted_ancestors)})
        elif filter_mode == FILTER_WHITELIST and not is_file: # A directory that is NOT explicitly whitelisted itself
            # BUT, it might *contain* whitelisted items (files or subfolders) further down.
            # So, we need to check if it's a potential parent of any whitelisted item.
            can_contain_whitelisted = False
            for rf_path in rules_files: # Check against whitelisted files
                if rf_path.startswith(normalized_item_path + os.sep):
                    can_contain_whitelisted = True
                    break
            if not can_contain_whitelisted:
                for rfo_path in rules_folders: # Check against whitelisted folders
                    if rfo_path == normalized_item_path or rfo_path.startswith(normalized_item_path + os.sep) : # direct match or subfolder
                        can_contain_whitelisted = True
                        break
            if can_contain_whitelisted:
                 # Pass current_whitelisted_ancestors, as this folder itself isn't adding to the direct whitelist scope yet
                 dirs_to_recurse_info.append({'name': item_name, 'path': normalized_item_path, 'ancestors': list(current_whitelisted_ancestors)})
    ...
    # Recurse into subdirectories that were deemed processable or potentially contain processable items
    processed_subdirs_with_content = []
    for dir_info in dirs_to_recurse_info:
        # The 'ancestors' for the next level are those passed to this dir_info
        next_level_ancestors = list(dir_info['ancestors']) # Use the ancestors determined when dir_info was created
        # If the directory we are about to recurse into (dir_info['path']) IS itself a whitelisted folder,
        # it should be added to the next_level_ancestors list for *its* children's benefit.
        if filter_mode == FILTER_WHITELIST and dir_info['path'] in rules_folders:
            if dir_info['path'] not in next_level_ancestors: # Avoid duplicates
    ...
    # Determine if a header for the current directory should be written
    should_write_header = False
    is_current_dir_in_whitelisted_scope = False # Is this directory itself, or an ancestor, whitelisted?
    if filter_mode == FILTER_WHITELIST:
        if normalized_directory in rules_folders: # This dir is directly whitelisted
            is_current_dir_in_whitelisted_scope = True
        else: # Check if it's a descendant of a whitelisted folder passed down
            for wf_ancestor in (whitelisted_ancestor_folders if whitelisted_ancestor_folders else []): # Use the ones passed to this call
                if normalized_directory.startswith(wf_ancestor + os.sep):
                    is_current_dir_in_whitelisted_scope = True
                    break
    
    if filter_mode == FILTER_BLACKLIST:
        # Write header if there are files to list, or subdirs that had content,
        # or if it's the root directory and it's empty (to indicate it was scanned)
        ...
        # Write header if this directory is within a whitelisted scope AND has files/subdirs OR is empty
        if is_current_dir_in_whitelisted_scope and (files_to_output or processed_subdirs_with_content or not items):
            should_write_header = True
        # OR, if it's not in whitelisted scope itself, but contains files (must be individually whitelisted)
        elif files_to_output: # These files must have been explicitly whitelisted
             should_write_header = True
        # OR, if it's not in whitelisted scope, but contains subdirectories that had content
        elif processed_subdirs_with_content: # These subdirs led to whitelisted content
             should_write_header = True


    if not should_write_header:
        return content_written_for_this_branch # Nothing to write for this dir's header or its direct files
    ...
    elif not items and not processed_subdirs_with_content : # Empty and no processed subdirectories
        # Only show this message if the folder itself was meant to be included
...
class CodeScannerApp:
    def __init__(self, root):
        ...
        self.current_rules_filepath = tk.StringVar() # This stores the path to the loaded .scanIgnore or similar
        self.status_text = tk.StringVar()
        self.status_text.set("Initializing...")
        self.filter_mode_var = tk.StringVar(value=FILTER_BLACKLIST)

        self.rules_files = [] # In-memory list of full file paths for rules
        self.rules_folders = [] # In-memory list of full folder paths for rules
        self.rules_dirty = False # Tracks if in-memory rules differ from the loaded file

        self.profiles, self.last_active_profile_name = load_profiles()
        self.active_profile_name = None # Will be set if a profile is loaded
        ...
        profile_menu.add_command(label="Load Profile...", command=self._load_profile_dialog) # Opens manager
        profile_menu.add_command(label="Delete Profile...", command=self._delete_profile_dialog) # Opens manager
        ...
        self.rules_frame = ttk.LabelFrame(main_frame, text="Rules List", padding="5") # Title updated by _update_rules_frame_title
        ...
        self.rules_frame.rowconfigure(0, weight=1) # Make the canvas area expandable
        current_row += 1

        self.rules_canvas = tk.Canvas(self.rules_frame, borderwidth=0)
        self.rules_list_frame = ttk.Frame(self.rules_canvas) # This frame holds the actual rule widgets
        ...
        self.rules_list_frame.bind("<Configure>", self._on_frame_configure) # For scroll region
        self.rules_canvas.bind('<Configure>', self._on_canvas_configure) # For resizing the frame in canvas
        # Mouse wheel scrolling for main rules list
        self.root.bind_all("<MouseWheel>", self._on_mousewheel, add="+") # For Windows/some Linux
        self.root.bind_all("<Button-4>", self._on_mousewheel, add="+") # For Linux (scroll up)
        self.root.bind_all("<Button-5>", self._on_mousewheel, add="+") # For Linux (scroll down)
        ...
        self._initialize_rules_file_and_display() # Load rules based on current_rules_filepath (if any)
        ...
        except tk.TclError: # In case the theme doesn't support it or on some systems
        ...
        main_frame.columnconfigure(1, weight=1) # Make entry column expandable
        main_frame.rowconfigure(current_row - 2, weight=1) # Make rules list area expandable (row of self.rules_frame)
    ...
    def _configure_fields_from_initial_profile(self):
        ...
        if profile_to_load_name and profile_to_load_name in self.profiles:
            ...
            if profile_data:
                ...
                self.active_profile_name = profile_to_load_name # Set active profile
                configured_successfully = True

        if not configured_successfully:
            # Set to defaults if no profile or invalid profile was last active
            self.scan_directory.set("")
            self.save_filepath.set(os.path.join(os.path.expanduser("~"), DEFAULT_OUTPUT_FILENAME))
            self.current_rules_filepath.set("") # No default rules file initially
            self.filter_mode_var.set(FILTER_BLACKLIST)
            self.active_profile_name = None


    def _on_frame_configure(self, event=None):
        # Update scroll region of the canvas when the frame inside it changes size
        self.rules_canvas.configure(scrollregion=self.rules_canvas.bbox("all"))

    def _on_canvas_configure(self, event):
        # Adjust the width of the frame within the canvas to match the canvas width
        self.rules_canvas.itemconfig(self.canvas_window, width=event.width)

    def _on_mousewheel(self, event):
        # Determine which canvas should be scrolled based on mouse position
        canvas_to_scroll = None
        x_root, y_root = event.x_root, event.y_root
        widget_under_mouse = self.root.winfo_containing(x_root, y_root)

        # Traverse up from the widget under mouse to find a known scrollable canvas
        current_widget = widget_under_mouse
        while current_widget is not None:
            if current_widget == self.rules_canvas: # Main rules canvas
                canvas_to_scroll = self.rules_canvas
                break
            # Check if inside EditDefaultsDialog and over its specific canvas
            if isinstance(current_widget, Toplevel): # Check if it's any Toplevel
                if hasattr(current_widget, 'dialog_rules_canvas') and current_widget.dialog_rules_canvas.winfo_ismapped():
                    ...
                    # Check if mouse is within the bounds of this dialog's canvas
                    ...
                    if cx <= x_root < cx + cw and cy <= y_root < cy + ch:
                        ...
                        break
            if current_widget == self.root: break # Reached the main window, stop
            current_widget = current_widget.master # Go to parent widget

        if canvas_to_scroll:
            if event.num == 5 or event.delta < 0: # Scroll down (Windows/Linux)
                canvas_to_scroll.yview_scroll(1, "units")
            elif event.num == 4 or event.delta > 0: # Scroll up (Windows/Linux)
                canvas_to_scroll.yview_scroll(-1, "units")
    ...
    def _update_status(self, message, clear_after_ms=None):
        ...
        if clear_after_ms:
            # If a clear is scheduled, ensure it only clears *this* message
    ...
    def _on_filter_mode_change(self):
        ...
        self._rebuild_rules_list_gui() # Rules display might change (Include vs Exclude)
    ...
    def _save_profile_dialog(self):
        ...
        if profile_name:
            profile_name = "".join(c for c in profile_name if c.isalnum() or c in (' ', '_', '-')).strip() # Basic sanitization
            if not profile_name:
            ...
            rules_fp = self.current_rules_filepath.get() # Path to .scanIgnore file
            ...
            if not rules_fp or not os.path.isfile(rules_fp) : # Rule file must exist to be part of a profile
            ...
            self.profiles[profile_name] = {
                ...
                "rules_filepath": os.path.normpath(rules_fp), # Save path to the rules file
                "filter_mode": filter_mode
            }
            self.last_active_profile_name = profile_name
            self.active_profile_name = profile_name # Set current profile as active
    ...
    def _apply_profile_settings(self, profile_name, persist_last_active=False):
        ...
        if profile_data:
            ...
            # If switching rules files and current rules are dirty, confirm discard
            if normalized_new_rules_path != normalized_current_rules_path and self.rules_dirty:
                if messagebox.askyesno("Unsaved Rules", f"You have unsaved changes in the current rules list for\n'{os.path.basename(normalized_current_rules_path if normalized_current_rules_path else 'Untitled Rules')}'.\nDiscard changes to load rules from the new profile?", default=messagebox.NO, parent=self.root):
                    ...
                    return False # Abort loading profile

            self.current_rules_filepath.set(new_rules_filepath)
            self._initialize_rules_file_and_display() # This loads rules from the file specified in profile & resets dirty

            self.active_profile_name = profile_name # Set current profile as active
            if persist_last_active: # Usually true when loading a profile explicitly
                self.last_active_profile_name = profile_name
                save_profiles(self.profiles, self.last_active_profile_name)
            ...
            self.active_profile_name = None # Clear active profile if load failed
            self._update_window_title()
            return False
    ...
    def _manage_profiles_dialog(self, initial_action="load"): # Can be "load" or "delete"
        if not self.profiles and initial_action != "save": # "save" is not an action for this dialog
            messagebox.showinfo("No Profiles", "No profiles saved yet to manage.", parent=self.root)
            return
        # Pass `self` (the CodeScannerApp instance) to the dialog
    ...
    def _execute_delete_profile(self, profile_name_to_delete):
        ...
        if profile_name_to_delete in self.profiles:
            ...
            if self.last_active_profile_name == profile_name_to_delete:
                self.last_active_profile_name = None # Clear if it was the last active one
            if self.active_profile_name == profile_name_to_delete:
                # If the currently active profile is deleted, clear relevant fields
                self.active_profile_name = None
                self.scan_directory.set("")
                self.save_filepath.set(os.path.join(os.path.expanduser("~"), DEFAULT_OUTPUT_FILENAME))
                self.current_rules_filepath.set("")
                self._clear_and_display_rules_list(None) # Clear rules from GUI as well
                self._update_status("Active profile was deleted. Please configure or load another.", clear_after_ms=5000)

            save_profiles(self.profiles, self.last_active_profile_name) # Save changes to profiles.json
            self._update_window_title()
            return True # Deletion successful
        return False # Profile not found (should not happen if called from dialog properly)
    ...
    def _browse_save_file(self):
        ...
        # Suggest initial directory based on current save path, or scan dir, or user's home
        ...
        if not initial_dir_suggestion:
            ...
            if scan_dir and os.path.isdir(scan_dir):
                initial_dir_suggestion = scan_dir # Sensible default if scan dir is set
            else:
    ...
    def _initialize_rules_file_and_display(self):
        ...
        if filepath:
            ...
            if os.path.exists(normalized_filepath) and os.path.isfile(normalized_filepath):
                self._load_and_display_rules(normalized_filepath) # This resets rules_dirty = False
            else:
                # File path is set but file doesn't exist (e.g. from a profile with a deleted rules file)
                ...
                if prompt_create:
                    if create_empty_file(normalized_filepath):
                        self._clear_and_display_rules_list(normalized_filepath) # Empty rules, dirty = False
                    else: # Failed to create
                        self.current_rules_filepath.set("") # Clear the invalid path
                        self._clear_and_display_rules_list(None)
                else: # User chose not to create
                    self.current_rules_filepath.set("") # Clear the invalid path
                    self._clear_and_display_rules_list(None)
        else: # No rules file path is set
            self._clear_and_display_rules_list(None) # Clears rules and sets dirty = False
        self._update_rules_frame_title()


    def _browse_rules_file(self):
        """Allows user to select or create a rules file. Handles unsaved changes confirmation."""
        # Determine initial directory for the browse dialog
        initial_dir_browse = os.path.dirname(self.current_rules_filepath.get()) or \
                             self.scan_directory.get() or \
                             os.path.expanduser("~")

        filepath_selected = filedialog.asksaveasfilename( # Use asksaveasfilename to allow creating new
            title="Select or Create Path-Based Rules File",
            initialdir=initial_dir_browse,
            initialfile=".scanIgnore", # Default filename suggestion
        ...
        if filepath_selected:
            ...
            # If rules are dirty for the *current* file, ask to discard
            if self.rules_dirty: # Check self.rules_dirty before changing current_rules_filepath
                ...
                if not confirm_discard:
                    ...
                    return
                self.rules_dirty = False # User chose to discard changes

            # Proceed to set new file path and load/initialize it
            self.current_rules_filepath.set(normalized_new_filepath) # Update the variable
            if os.path.exists(normalized_new_filepath) and os.path.isfile(normalized_new_filepath):
                self._load_and_display_rules(normalized_new_filepath) # Loads rules, resets dirty flag
            else: # New file path, or path to a non-file/non-existent location (asksaveasfilename implies intent to create)
                if create_empty_file(normalized_new_filepath): # Try to create it
                    self._clear_and_display_rules_list(normalized_new_filepath) # Sets up for a new empty file, resets dirty
                else: # Failed to create the file (e.g. permissions)
                    # Revert to old path if creation failed, and re-init display for that
                    self.current_rules_filepath.set(current_normalized_path if current_normalized_path else "")
                    self._initialize_rules_file_and_display() # Re-load or clear based on the reverted path
                    self._update_status("Failed to create new rules file. Selection reverted.", clear_after_ms=4000)
            self._update_rules_frame_title() # Update title based on new path and dirty status
        else:
    ...
    def _update_rules_frame_title(self):
        ...
        dirty_star = "*" if self.rules_dirty else "" # Add asterisk if rules are modified and unsaved

        if filepath:
            title = f"Rules: {os.path.basename(filepath)}{dirty_star} {mode_text}"
        else:
            title = f"Rules (No file selected){dirty_star} {mode_text}" # Still show dirty if no file but rules modified
    ...
    def _load_and_display_rules(self, filepath):
        ...
        try:
            ...
            self.rules_dirty = False # Crucial: loading from file means rules are no longer dirty
            ...
            self._clear_and_display_rules_list(filepath) # Clear if load failed, but keep context of file path
        finally:
            self._update_rules_frame_title() # Update title, including dirty status
    ...
    def _clear_and_display_rules_list(self, filepath_context):
        ...
        self.rules_dirty = False # Clearing rules also means they are not "dirty" compared to an empty state
        self._rebuild_rules_list_gui()

        if filepath_context: # If clearing in context of a specific file (e.g. new empty file)
            self._update_status(f"Rules list for {os.path.basename(filepath_context)} is now empty. Ready.", clear_after_ms=4000)

        else: # No file context (e.g. no file selected)
            self._update_status("No rules file loaded. Rules list cleared.")
        self._update_rules_frame_title() # Update title, including dirty status


    def _rebuild_rules_list_gui(self):
        if not hasattr(self, 'rules_list_frame'): # Safety check
            print("Warning: _rebuild_rules_list_gui called before rules_list_frame was initialized.")
            return

        # Clear existing rule widgets from the scrollable frame
        for widget in self.rules_list_frame.winfo_children():
            widget.destroy()

        row_num = 0
        current_file_path = self.current_rules_filepath.get() # For placeholder text
        mode = self.filter_mode_var.get()
        rule_action_text = "Include Path" if mode == FILTER_WHITELIST else "Exclude Path"

        # Display File Rules
        if self.rules_files: # Check if the list itself is not empty
            ...
            row_num += 1
            for path_rule in sorted(self.rules_files): # Ensure sorted display
                item_frame = ttk.Frame(self.rules_list_frame)
                item_frame.grid(row=row_num, column=0, columnspan=2, sticky=tk.EW, padx=5, pady=1)
                item_frame.columnconfigure(0, weight=1) # Label takes available space

                label_text = f"file: {path_rule}" # Full path for tooltip
                max_len = 75 # Max length for display in label
                display_text = label_text if len(label_text) <= max_len else "..." + label_text[-(max_len-3):]
                tooltip_text = label_text if len(label_text) > max_len else None # Tooltip only if text was truncated
        ...
        if self.rules_folders: # Check if the list itself is not empty
            ...
            row_num += 1
            for path_rule in sorted(self.rules_folders): # Ensure sorted display
                item_frame = ttk.Frame(self.rules_list_frame)
                item_frame.grid(row=row_num, column=0, columnspan=2, sticky=tk.EW, padx=5, pady=1)
                item_frame.columnconfigure(0, weight=1) # Label takes available space

                label_text = f"folder: {path_rule}" # Full path for tooltip
                max_len = 75 # Max length for display in label
                display_text = label_text if len(label_text) <= max_len else "..." + label_text[-(max_len-3):]
                tooltip_text = label_text if len(label_text) > max_len else None # Tooltip only if text was truncated
        ...
        # Style for the small remove buttons
        style = ttk.Style()
        try: style.configure("Small.TButton", padding=(1, 1), font=('TkDefaultFont', 7))
        except tk.TclError: pass # Ignore if style not applicable

        # Placeholder if no rules
        ...
        self.rules_list_frame.update_idletasks() # Ensure frame is updated for bbox
        self._on_frame_configure() # Update scrollregion
        self._update_rules_frame_title() # Reflect dirty status in title if changed
    ...
    def _remove_rule_item(self, item_type, path_pattern):
        ...
        if removed:
            self.rules_dirty = True # Mark rules as dirty
            self._rebuild_rules_list_gui() # This will also call _update_rules_frame_title
            self._update_status(f"Removed rule for '{os.path.basename(path_pattern)}'. Save changes to persist.", clear_after_ms=4000)
        else: # Should not happen if called from GUI correctly
    ...
    def _add_file_rules(self):
        ...
        if not self.current_rules_filepath.get(): # Require a rules file to be "active" even for temporary rules
        ...
        # Suggest initial directory based on scan directory or user home
        ...
        for fp_raw in filepaths_to_add:
            normalized_fp_add = os.path.normpath(fp_raw) # Normalize before adding
            if normalized_fp_add and normalized_fp_add not in self.rules_files:
                self.rules_files.append(normalized_fp_add)
                added_count += 1

        if added_count > 0:
            self.rules_files.sort() # Keep the internal list sorted
            self.rules_dirty = True # Mark rules as dirty
            self._rebuild_rules_list_gui() # This will also call _update_rules_frame_title
    ...
    def _add_folder_rule(self):
        ...
        if not self.current_rules_filepath.get(): # Require a rules file to be "active"
            messagebox.showwarning("No Rules File Active", "Please select or create a rules file first before adding rules to it.", parent=self.root)
            return
        ...
        normalized_fp_add_folder = os.path.normpath(folderpath_to_add) # Normalize before adding
        if normalized_fp_add_folder and normalized_fp_add_folder not in self.rules_folders:
            ...
            self.rules_folders.sort() # Keep internal list sorted
            self.rules_dirty = True # Mark rules as dirty
            self._rebuild_rules_list_gui() # This will also call _update_rules_frame_title
        ...
        elif normalized_fp_add_folder: # Folder was selected, but was already in list
    ...
    def _save_rules_list_changes(self):
        ...
        if not current_path_to_save: # No file selected to save to
        ...
        try:
            ...
            self.rules_dirty = False # Changes are now saved
            self._rebuild_rules_list_gui() # This will also call _update_rules_frame_title to remove asterisk
    ...
    def _run_scan_thread(self, scan_dir_norm, save_path_norm, rules_files_for_scan, rules_folders_for_scan, filter_mode_val, used_rules_file_display_path):
        """
        This is the actual scan operation running in a separate thread.
        It uses the rules passed to it directly.
        `used_rules_file_display_path` is for reporting purposes only.
        """
        try:
            with open(save_path_norm, "w", encoding="utf-8") as output_file:
                ...
                # Display the rules file path if one is associated, otherwise indicate no file or temporary rules
                rules_source_display = f"`{os.path.basename(used_rules_file_display_path)}` (from `{used_rules_file_display_path}`)" if used_rules_file_display_path else "`Current GUI rules (No file or unsaved changes)`"
                if self.rules_dirty and used_rules_file_display_path: # Add a note if rules are from a file but modified
                    rules_source_display += " - with unsaved modifications"

                output_file.write(f"**Rules From:** {rules_source_display}\n\n")

                initial_whitelisted_ancestors = []
                # If the root scan directory itself is a whitelisted folder, add it to initial ancestors
            ...
            # --- Scan Completion (runs on main thread via self.root.after) ---
            def on_scan_complete(): # This will be scheduled to run in the Tkinter main loop
            ...
            # --- Scan Error Handling (runs on main thread) ---
            def on_scan_error(): # Scheduled to run in Tkinter main loop
                error_message = f"An error occurred during scanning or writing: {e}"
                print(f"Full scan error details: {error_message}") # Log full error to console
                import traceback
    ...
    def _run_scan(self):
        ...
        # rules_path_ui_current is the path to the *file* on disk, if one is loaded.
        # The scan will use self.rules_files and self.rules_folders which are the live GUI rules.
        ...
        # A rules file path in the GUI is no longer strictly required to *run* a scan,
        # as we use in-memory rules. However, it's good practice to have one associated
        # if the user intends for rules to be based on a file.
        # For now, we'll allow scanning even if rules_path_ui_current is empty,
        # which implies the rules are purely in-memory (e.g., added manually without loading a file).

        # However, if a rules file IS specified, it should ideally exist for clarity,
        # though the scan uses in-memory rules. The `used_rules_file_display_path` will reflect this.
        normalized_rules_path_for_display = os.path.normpath(rules_path_ui_current) if rules_path_ui_current else None

        # If a rules file path *is* provided, but doesn't exist, it might be confusing.
        # For now, we'll proceed, as the scan uses in-memory rules.
        # The output log will indicate if the rules file was "None" or had unsaved changes.
        # A warning could be added here if rules_path_ui_current is set but file not found,
        # but it's not a blocker for scanning with in-memory rules.
        ...
        # MODIFICATION: Removed prompt to save dirty rules before scan.
        # The scan will now ALWAYS use the current self.rules_files and self.rules_folders from memory.
        # The self.rules_dirty flag and asterisk in GUI will still indicate if these differ from the file.

        # Prepare data for the scan thread using current in-memory rules
        # These are already normalized when added/loaded.
        ...
        # The rules source for display will be determined within _run_scan_thread based on
        # rules_path_ui_current and self.rules_dirty status at the time of scan.
        status_msg_scan = f"Starting {mode_text_display_scan} scan using current GUI rules."
        if normalized_rules_path_for_display:
            status_msg_scan += f" Associated file: {os.path.basename(normalized_rules_path_for_display)}"
            if self.rules_dirty:
                status_msg_scan += " (has unsaved changes)"
        else:
            status_msg_scan += " (No rules file associated or rules are entirely in-memory)"
        ...
        scan_thread = threading.Thread(
            ...
            args=(norm_scan_dir_for_thread, norm_save_path_for_thread,
                  scan_rules_files_mem, scan_rules_folders_mem, # Pass in-memory rules
                  filter_mode_selected, normalized_rules_path_for_display), # Pass original rules file path for display
            daemon=True
        )
        scan_thread.start()
...
class EditDefaultsDialog(Toplevel):
    def __init__(self, parent, default_filepath, app_instance):
        ...
        self.default_filepath = default_filepath # Path to .scanIgnore.defaults
        self.app_instance = app_instance # To call _update_status on main app

        self.title(f"Edit Default Name Patterns ({os.path.basename(default_filepath)})")
        self.geometry("600x500") # Adjusted size
        self.transient(parent) # Keep on top of parent
        self.grab_set() # Modal behavior

        self.dialog_rule_file_patterns = [] # Store name patterns for files
        self.dialog_rule_folder_patterns = [] # Store name patterns for folders
        ...
        # Frame for scrollable list of default patterns
        ...
        dialog_rules_frame.rowconfigure(0, weight=1) # Canvas expands

        self.dialog_rules_canvas = tk.Canvas(dialog_rules_frame, borderwidth=0)
        self.dialog_rules_list_frame = ttk.Frame(self.dialog_rules_canvas) # Holds pattern widgets
        ...
        self.dialog_rules_canvas.grid(row=0, column=0, sticky="nsew") # Canvas fills its cell
        self.dialog_rules_scrollbar.grid(row=0, column=1, sticky="ns") # Scrollbar next to canvas

        self.dialog_canvas_window = self.dialog_rules_canvas.create_window((0, 0), window=self.dialog_rules_list_frame, anchor="nw")

        self.dialog_rules_list_frame.bind("<Configure>", self._on_dialog_list_frame_configure) # Update scrollregion
        self.dialog_rules_canvas.bind('<Configure>', self._on_dialog_canvas_viewport_configure) # Adjust frame width in canvas
        # Specific mouse wheel binding for this dialog's canvas
        self.dialog_rules_canvas.bind("<MouseWheel>", self._on_dialog_specific_mousewheel)
        self.dialog_rules_canvas.bind("<Button-4>", self._on_dialog_specific_mousewheel) # Linux scroll up
        self.dialog_rules_canvas.bind("<Button-5>", self._on_dialog_specific_mousewheel) # Linux scroll down
        ...
        self._load_initial_name_patterns() # Load and display existing patterns
        self.wait_window(self) # Make dialog modal


    # --- Methods for Scrollable Canvas in EditDefaultsDialog ---
    ...
    def _on_dialog_specific_mousewheel(self, event): # Handles scrolling only for this dialog's canvas
    ...
    def _load_initial_name_patterns(self):
        ...
        try:
            # Create default .scanIgnore.defaults if it doesn't exist
            if not os.path.exists(self.default_filepath): # Check SCRIPT_DIR location
    ...
    def _rebuild_dialog_name_patterns_list(self):
        ...
        for widget in self.dialog_rules_list_frame.winfo_children(): # Ensure this is the correct frame
            widget.destroy()
        row_num = 0
        ...
        if self.dialog_rule_file_patterns:
            ...
            row_num += 1
            for pattern_name in self.dialog_rule_file_patterns: # Already sorted
        ...
        if self.dialog_rule_folder_patterns:
            ...
            row_num += 1
            for pattern_name in self.dialog_rule_folder_patterns: # Already sorted
                item_f = ttk.Frame(self.dialog_rules_list_frame)
                item_f.grid(row=row_num, column=0, columnspan=2, sticky=tk.EW, padx=5, pady=1); item_f.columnconfigure(0, weight=1)
        ...
        self.dialog_rules_list_frame.update_idletasks() # Ensure layout is updated
        self._on_dialog_list_frame_configure() # Recalculate scroll region

    def _dialog_add_name_pattern(self, item_type_str): # 'file' or 'folder'
        ...
        if item_type_str == 'file':
            if pattern_to_add not in self.dialog_rule_file_patterns:
                ...
                self.dialog_rule_file_patterns.sort() # Keep sorted
            ...
            if pattern_to_add not in self.dialog_rule_folder_patterns:
                ...
                self.dialog_rule_folder_patterns.sort() # Keep sorted
                pattern_added_successfully = True

        if pattern_added_successfully:
            self._rebuild_dialog_name_patterns_list()
            self.pattern_entry_var.set("") # Clear entry field
        else: # Pattern already exists
            messagebox.showinfo("Pattern Exists", f"The pattern '{pattern_to_add}' is already in the list for its type.", parent=self)


    def _dialog_remove_name_pattern(self, item_type_str, pattern_to_remove): # 'file' or 'folder'
    ...
    def _save_and_close_name_patterns(self):
        try:
            # Save the patterns to the .scanIgnore.defaults file
            with open(self.default_filepath, "w", encoding="utf-8") as f:
                f.write("# Default files to ignore (name patterns, e.g., *.log, specific_file.tmp)\n")
            ...
            # Update status on main app window if possible
...
class ManageProfilesDialog(Toplevel):
    def __init__(self, parent, profiles_data, current_active_profile, app_instance, initial_action="load"):
        super().__init__(parent)
        self.profiles_dict = profiles_data # Reference to the app's profiles dictionary
        self.app_instance_ref = app_instance # To call _execute_load_profile or _execute_delete_profile
        ...
        self.profile_listbox_widget = tk.Listbox(main_frame, height=10, exportselection=False) # exportselection=False is important
        self.profile_listbox_widget.pack(fill=tk.BOTH, expand=True, pady=5)
        self._populate_profile_listbox() # Fill with current profiles
        ...
        close_button_widget.pack(side=tk.RIGHT, padx=5) # Align to right

        self._update_button_states() # Enable/disable based on list content

        # Set initial focus based on action
        ...
        self.profile_listbox_widget.bind("<Double-1>", lambda e: self._action_load_selected()) # Double-click to load
        self.wait_window(self) # Make modal
    ...
    def _populate_profile_listbox(self):
        ...
        selection_to_set_idx = -1 # To pre-select the active profile

        for idx, name in enumerate(sorted_profile_names):
            display_name = name
            if name == self.current_active_profile_name: # Mark active profile
                display_name += " (Active)"
                selection_to_set_idx = idx # Note its index for selection
            self.profile_listbox_widget.insert(tk.END, display_name)
        
        if selection_to_set_idx != -1: # If an active profile was found
        ...
        elif sorted_profile_names: # Otherwise, select the first item if list is not empty
            self.profile_listbox_widget.selection_set(0)
            self.profile_listbox_widget.see(0)


    def _update_button_states(self):
        # Disable load/delete if no profiles or no selection
        if not self.profiles_dict: # or not self.profile_listbox_widget.curselection():
    ...
    def _get_actual_profile_name_from_listbox_selection(self):
        ...
        if not selection_indices: return None # No selection
        
        selected_display_name = self.profile_listbox_widget.get(selection_indices[0])
        # Remove " (Active)" suffix to get the actual profile name key
    ...
    def _action_load_selected(self):
        ...
        if profile_name_to_load and self.app_instance_ref:
            self.app_instance_ref._execute_load_profile(profile_name_to_load) # Call main app's method
            self.destroy() # Close dialog after loading
        ...
        if profile_name_to_delete:
            if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete profile '{profile_name_to_delete}'?", parent=self):
                if self.app_instance_ref and self.app_instance_ref._execute_delete_profile(profile_name_to_delete):
                    # Refresh the dialog's view of profiles
                    self.profiles_dict = self.app_instance_ref.profiles # Update local ref
                    self.current_active_profile_name = self.app_instance_ref.active_profile_name # Update active status
                    self._populate_profile_listbox() # Re-draw listbox
                    self._update_button_states() # Re-check button states
                    # Status update is handled by _execute_delete_profile in main app
                    self.app_instance_ref._update_status(f"Profile '{profile_name_to_delete}' deleted.", clear_after_ms=4000)
        elif not profile_name_to_delete: # No selection
...
class ToolTip:
    def __init__(self, widget, text):
        self.widget = widget
        self.text = text
        self.tooltip_window = None # Holds the Toplevel window for the tooltip
        self.id_after = None       # ID for self.widget.after scheduled task
        self.widget.bind("<Enter>", self.on_enter)
        self.widget.bind("<Leave>", self.on_leave)
        self.widget.bind("<ButtonPress>", self.on_leave) # Hide on click too
    ...
    def schedule_tooltip(self):
        self.cancel_scheduled_tooltip() # Cancel any existing scheduled tooltip
        self.id_after = self.widget.after(700, self.show_tooltip) # Schedule to show after 700ms
    ...
    def show_tooltip(self):
        if self.tooltip_window or not self.text: return # Don't show if already visible or no text

        # Calculate position for tooltip
        x_root = self.widget.winfo_rootx() + 20 # Offset from widget
        y_root = self.widget.winfo_rooty() + self.widget.winfo_height() + 5 # Below the widget
        ...
        if x_root + tip_width > screen_width: # If extends past right edge
            x_root = screen_width - tip_width - 5 # Move to stay on screen
        if x_root < 0: x_root = 5 # Ensure not off left edge

        if y_root + tip_height > screen_height: # If extends past bottom edge
            y_root = self.widget.winfo_rooty() - tip_height - 5 # Move above widget
        if y_root < 0: y_root = 5 # Ensure not off top edge

        self.tooltip_window.wm_geometry(f"+{int(x_root)}+{int(y_root)}") # Apply adjusted position

    def hide_tooltip(self):
        if self.tooltip_window:
            try:
                self.tooltip_window.destroy()
            except tk.TclError: # Can happen if widget/parent is destroyed first
                pass 
